...
def plot_bestmatch_results(ts: np.ndarray, query: np.ndarray, bestmatch_results: dict) -> None:
    ...
    # Цвета для topK совпадений
    topk_colors = px.colors.qualitative.Plotly
    indices = bestmatch_results['indices']

    for i, start_idx in enumerate(indices):
        start_idx_int = int(start_idx)
        match_segment = ts[start_idx_int:start_idx_int + query_len]

        fig.add_trace(go.Scatter(
            x=np.arange(start_idx_int, start_idx_int + query_len),
            y=match_segment,
            line=dict(color=topk_colors[i % len(topk_colors)], width=3),
            name=f"Match at {start_idx_int}"
        ), row=1, col=2)
